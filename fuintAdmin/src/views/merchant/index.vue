<template>
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" width="220" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['system:account:index']"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="微信公众号appSecret" prop="wxOfficialAppSecret">
              <el-input v-model="form.wxOfficialAppSecret" placeholder="请输入微信公众号appSecret，30-50个字符" maxlength="50"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            ...
            >
              <el-input
                ...
                placeholder="请输入结算比例，80%则输入80"
                maxlength="50"
              />
              <p class="form-tips">温馨提示：结算比例，80%则输入80</p>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    <!-- 门店列表弹窗 -->
    <el-dialog :title="'门店列表 - ' + (currentMerchant ? currentMerchant.name : '')" :visible.sync="storeDialogVisible" width="900px" append-to-body>
      <el-table :data="storeList" v-loading="storeLoading" style="width: 100%">
        <el-table-column prop="id" label="ID" width="60" />
        <el-table-column prop="name" label="店铺名称" min-width="120" />
        <el-table-column prop="isDefault" label="默认店铺" width="80" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column prop="status" label="状态" width="80" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.status === 'A' ? 'success' : 'danger'" size="small">
              {{ scope.row.status === 'A' ? '启用' : '禁用' }}
            </el-tag>
          </template>
        </el-table-column>
      </el-table>
      <div v-if="storeList.length === 0 && !storeLoading" style="text-align: center; padding: 20px; color: #909399;">
        暂无门店数据
      </div>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      typeOptions: [],
      // 表单参数
      form: { id: '', name: '', no: '', logo: '', contact: '', phone: '', wxAppId: '', wxAppSecret: '', wxOfficialAppId: '', wxOfficialAppSecret: '', description: '', status: "A", settleRate: "" },
      // 上传地址
  ...
  methods: {
    ...
    handleViewStores(row) {
      ...
      this.storeList = [];
      searchStore({ merchantId: row.id }).then(response => {
        this.storeList = response.data.storeList || [];
        this.storeLoading = false;
      }).catch(() => {
        this.storeLoading = false;
        this.$modal.msgError("获取门店列表失败");
      });
    }
  }
};
