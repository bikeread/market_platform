<template>
  <div class="app-container">
    <el-form :model="queryParams" class="main-search" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item>
        <dev class="form-tips clearfix">温馨提示：订单结算是平台统一收款，再给商家结算的运营模式。</dev>
      </el-form-item>
    </el-form>
    ...
    <el-table ref="tables" v-loading="loading" :data="list" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="结算比例" align="center" prop="settleRate">
        <template slot-scope="scope">
          <span>{{ scope.row.settleRate ? scope.row.settleRate.toFixed(2) : 100 }}%</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="发起结算" :visible.sync="settleDialog" class="common-dialog" width="80%" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <p class="form-tips">温馨提示：仅结算微信、支付宝支付的订单</p>
    ...
    <el-dialog title="结算详情" :visible.sync="detailDialog" class="common-dialog" width="80%" append-to-body>
      <el-table ref="tables" v-loading="loading" :data="settlementInfo ? settlementInfo.orderList.content : []">
        ...
        <el-table-column label="支付金额" align="center" prop="payAmount">
          <template slot-scope="scope">
            <span v-if="true">{{ scope.row.orderInfo.payAmount.toFixed(2) }}</span>
            <span v-else>0.00</span>
          </template>
        </el-table-column>
        <el-table-column label="下单时间" align="center" width="130" prop="createTime">
...
export default {
  ...
  data() {
    return {
      ...
      form: { page: 1, pageSize: 10, merchantId: this.$store.getters.merchantId, storeId: '', startTime: '', endTime: '', status: '', payType: ''}
    };
  },
  created() {
    this.getList();
  ...
  methods: {
    ...
    queryOrder() {
      ...
      app.form.status = 'I';
      app.form.payType = ['JSAPI', 'MICROPAY', 'ALISCAN'];
