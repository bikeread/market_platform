<template>
  <div class="sku-list">
    ...
    <div class="batch-setting">
      ...
      <el-input size="small" v-model="batch.costPrice" placeholder="成本价格" class="input"></el-input>
    ...
    <el-table border :data="skuData.skuList">
      ...
      <el-table-column label="划线价格(元)" align="center">
        <template slot-scope="scope">
          <el-input :readonly="disabled" v-model="scope.row.linePrice"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="成本价格(元)" align="center">
        <template slot-scope="scope">
          <el-input :readonly="disabled" v-model="scope.row.costPrice"></el-input>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      // 批量设置
      batch: { price: '', skuNo: '', linePrice: '', costPrice: '', weight: '', stock: '' }
    };
  },
  watch: {
    "skuData.attrList": {
  ...
  methods: {
    ...
    batchSetSku() {
      ...
      app.skuData.skuList.forEach(function(skuInfo, index) {
        ...
        if (app.batch.costPrice) {
          if (app.batch.costPrice >= 0) {
            skuInfo.costPrice = app.batch.costPrice
          } else {
            app.$modal.alert('商品成本价格须大于等于0！')
            return false
          }
        }
         if (app.batch.weight) {
