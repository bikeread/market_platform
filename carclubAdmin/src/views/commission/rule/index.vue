  ...
  <div class="app-container">
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="126px">
        ...
        <div v-if="form.type == 'recharge' || form.type == 'payment'">
          <el-row>
            <el-col :span="24">
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="散客比例" prop="visitorVal">
                <el-input
                  v-model="form.visitorVal"
                  placeholder="请输入散客比例值"
                  maxlength="90"
                  style="width: 25%"
                />
              </el-form-item>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="会员比例" prop="memberVal">
                <el-input
                  v-model="form.memberVal"
                  placeholder="请输入会员比例值"
                  maxlength="90"
                  style="width: 25%"
                />
              </el-form-item>
      ...
      <div class="item-config" v-if="form.type == 'goods'">
        ...
        >
          ...
          <el-table-column prop="method" label="提成方式" width="136">
            <template slot-scope="scope">
              <select
                :rowId="scope.row.goodsId"
                tp="method"
                style="border: 1px solid #dbdee5"
              >
                <option
                  :selected="scope.row.method == 'percent' ? true : false"
                  value="percent"
                >
                  按比率提成（%）
                </option>
                <option
                  :selected="scope.row.method == 'fixed' ? true : false"
                  value="fixed"
                >
                  按固定提成（元）
                </option>
          ...
          <el-table-column label="操作" width="77">
            <template slot-scope="scope">
              <el-button
                size="mini"
                @click="removeRule(scope.row.goodsId)"
                slot="reference"
                type="text"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        storeIdList: [
          { required: true, message: '请选择方案适用门店', trigger: 'blur' },
        ],
        name: [{ required: true, message: '请输入方案名称', trigger: 'blur' }],
        target: [
          { required: true, message: '请选择方案对象', trigger: 'blur' },
        ],
    ...
    selectOk() {
      ...
      this.multipleSelection.forEach((item) => {
        let detailObj = {
          ...
          visitorVal: 0.0, //散客（提成值）
          memberVal: 0.0, //会员（提成值）
          type: item.type, //提成类型
