              ...
              <el-row class="line">
                <div class="date-picker">
                  <el-select
                    ...
                    placeholder="所属店铺"
                  >
                    <el-option :key="0" label="全部店铺" :value="0"/>
...
<script>
import { getNumDayTime } from "@/utils/carclub";
...
export default {
  ...
  data() {
    return {
       ...
       chart2: { title: '近七日会员活跃数', color: '#373F64', chartType: 'line', header: ['会员统计'] },
       chartData1: [],
       chartData2: [],
       goodsList: [],
       memberList: [],
  ...
  methods: {
    ...
    getMainData() {
      this.loading = true;
      const param = { storeId: this.storeId, startTime: this.startTime, endTime: this.endTime }
    ...
    getChartsData() {
      ...
      app.loading = true;
      // 近7日订单数量和活跃会员数量
      getStatisticData({ storeId: this.storeId, tag : 'order,user_active' }).then(response => {
