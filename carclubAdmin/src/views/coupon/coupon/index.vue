<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" class="common-dialog" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="140px">
        ...
        <div v-if="form.type == 'C'">
          <el-row>
            <el-col :span="24">
              <el-form-item label="卡券内容" prop="content">
                <el-select v-model="form.content" style="width: 240px" placeholder="卡券内容">
                  <el-option :key="1" label="满减券" :value="1"/>
                  <el-option :key="2" label="折扣券" :value="2"/>
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item label="卡券面额" v-if="form.content == 1" prop="amount">
                <el-input v-model="form.amount" style="width: 240px" placeholder="请输入卡券面额，单位：元" maxlength="10" />
              </el-form-item>
              <el-form-item v-if="form.content == 2" label="卡券折扣" prop="amount">
                <el-input
                  v-model="form.amount"
                  style="width: 240px"
                  placeholder="请输入卡券折扣，如8折则输入80"
                  maxlength="10"
                />
                <p class="form-tips">温馨提示：8折则输入80</p>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
    ...
    <!--二维码对话框start-->
    <CarclubQrCode :showDialog="openQrCode" :qr="qr" @closeDialog="closeQrDialog"/>
    <!--二维码对话框end-->
  </div>
</template>

<script>
import CarclubQrCode from '@/components/Carclub/QrCode';
...
import { parseTime } from "@/utils/carclub";
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        content: [
          { required: true, message: '请选择卡券内容', trigger: 'blur' }
        ],
        name: [
  ...
  methods: {
    ...
    reset() {
      ...
      this.form = {
        ...
        useFor: '',
        storeIds: '',
        image: '/static/defaultImage/coupon.png',
        description: '',
        remarks: '',
        outRuleForTimer: '',
        status: 'A' };
